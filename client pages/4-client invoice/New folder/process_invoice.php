...
if(isset($_SESSION['loggedin']) && isset($_SESSION['role']) && $_SESSION['loggedin'] && $_SESSION['role'] == 'client') {
    ...
    // استخدم $name و $email لملء حقول النموذج
    echo "<script>
...
// التحقق مما إذا تم النقر على زر "Create Invoice"
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // السماح بالتسجيل
    insertInvoice($conn, $service_package, $collection_days, $collection_date, $name, $email, $password, $address, $zone, $street, $house_number, $postcode, $expiry_date);
...
// دالة لإدراج الفاتورة في قاعدة البيانات
function insertInvoice($conn, $service_package, $collection_days, $collection_date, $name, $email, $password, $address, $zone, $street, $house_number, $postcode, $expiry_date) {
    // استعد الاستعلام لإدراج البيانات في قاعدة البيانات مع تضمين تاريخ انتهاء الباقة
    $sql = "INSERT INTO invoices (service_package, collection_days, collection_date, name, email, password, address, zone, street, house_number, postcode, package_expiry_date) 
            VALUES ('$service_package', '$collection_days', '$collection_date', '$name', '$email', '$password', '$address', '$zone', '$street', '$house_number', '$postcode', '$expiry_date')";
