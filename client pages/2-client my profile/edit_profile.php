...
if(isset($_SESSION['loggedin']) && $_SESSION['loggedin'] === true){
    ...
    if ($result->num_rows > 0) {
        ...
        if ($row['ROLE'] === 'client') {
            // استخراج البيانات اللازمة للعرض
            $username = $row['username'];
            $email = $row['email'];
            $address = $row['address'];
            $phone = $row['phone'];
            $zone = $row['Zone'];
            $selected_package = isset($row['selected_package']) ? $row['selected_package'] : "";
        } else {
    ...
    header("Location: http://localhost:8080/garbage%20mangement%20project/2-login%20&sign%20for%20client/login_user.html");
    exit;
}
...
logout</span>Logout</a>
        ...
        <form action="update_profile.php" method="POST">
            ...
            <input type="text" id="zone" name="zone" value="<?php echo $zone; ?>" required><br>
            <!-- ازالة حقل Selected Package -->
            <!-- <label for="selected_package">Selected Package:</label><br>
            <input type="text" id="selected_package" name="selected_package" value="<?php echo $selected_package; ?>"><br> -->
