...
<html lang="en">
  ...
  if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($valid) {
        ...
        $checkEmailQuery = "SELECT email FROM collector WHERE email = '$email'";
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "INSERT INTO collector (username, email, password, address, phone, Zone)
            VALUES ('$username', '$email', '$password', '$address', '$phone', '$zone')";

            if ($conn->query($sql) === TRUE) {
                // تسجيل ناجح، تخزين رسالة النجاح في الجلسة
                $_SESSION['successMessage'] = "Collector added successfully!";
