...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($valid) {
        ...
        if ($result_check_user !== false && $result_check_user->num_rows > 0) {
            $emailError = "This email is already registered";
        } else {
            // إضافة بيانات التسجيل إلى قاعدة البيانات
            $sql_insert_user = "INSERT INTO users (username, email, password, address, phone, ROLE) 
...
logout</span>Logout</a>
   ...
   <div class="container">
    ...
    <form id="signupForm" class="form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="POST">
        ...
        <div class="error"><?php echo isset($usernameError) ? $usernameError : ''; ?></div>

        <label for="email">Email:</label>
        <input type="email" id="email" name="email" value="<?php echo isset($email) ? $email : ''; ?>" required>
        <div class="error"><?php echo isset($emailError) ? $emailError : ''; ?></div>

        <label for="password">Password:</label>
        <input type="password" id="password" name="password" required>
        <div class="error"><?php echo isset($passwordError) ? $passwordError : ''; ?></div>

        <label for="address">Address:</label>
        <input type="text" id="address" name="address" value="<?php echo isset($address) ? $address : ''; ?>" required>
        <div class="error"><?php echo isset($addressError) ? $addressError : ''; ?></div>

        <label for="phone">Phone:</label>
        <input type="text" id="phone" name="phone" value="<?php echo isset($phone) ? $phone : ''; ?>" required>
        <div class="error"><?php echo isset($phoneError) ? $phoneError : ''; ?></div>
